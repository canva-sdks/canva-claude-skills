...
description: Turn a lesson plan into a teaching slide deck in Canva. Use when the user asks to build classroom slides from a lesson plan, convert a lesson plan into a presentation, make a teaching deck, create school slides from an outline, or generate a lesson deck for students. Input can be a lesson plan pasted in the message, a Canva doc or design by name, or a Canva design link (e.g., https://www.canva.com/design/...).
---
...
1. **Get the lesson plan source** (always do this first — no substitute checklist)
   ...
   - If the user provides a **Canva design link** (e.g., `https://www.canva.com/design/DAG.../...`), extract the design ID and use `Canva:start-editing-transaction` to read its text content
...
**Where imagery comes from (not from this skill file)**  
Slide images are chosen or generated **inside Canva** when `Canva:generate-design` runs. The skill only supplies the **text `query`**. Canva’s side (layout + image selection and/or generative visuals) decides what appears on each slide. This repo does not host images or a separate illustration engine.
...
- If `Canva:list-brand-kits` returns none, ask whether to proceed without a brand kit per product behavior, or suggest connecting brand assets in Canva first
